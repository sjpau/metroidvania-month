...
class Melee(
    ...
    def __init__(self, parent, image, group, animations={}):
    ...
    def set_attack_hitbox_to_direction(self, attack_direction):
        if attack_direction['left']:
            self.rect.bottomright = self.parent.rect.bottomleft
            self.set_direction('left')
        elif attack_direction['right']:
            self.rect.topleft = self.parent.rect.topright # This sort of requires parent width to be same as attack box on for other directions respectively
            self.set_direction('right')
        if attack_direction['up']:
            self.rect.bottomright = self.parent.rect.topright
